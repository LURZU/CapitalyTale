...
public class ATMDepositPage extends InteractiveCustomUIPage<ATMDepositPage.Data> {
    ...
    public ATMDepositPage(PlayerRef playerRef, BankManager bankManager) {
    ...
    public void build(
        ...
        builder.append("ATMDeposit.ui");
        
        UUIDComponent uuidComponent = store.getComponent(ref, UUIDComponent.getComponentType());
        double balance = bankManager.getBalance(uuidComponent.getUuid());
        
        // Update balance displays using correct API
        builder.set("#CurrentBalance.TextSpans", Message.raw("$" + CURRENCY_FORMAT.format(balance)));
        builder.set("#BalanceAfter.TextSpans", Message.raw("$" + CURRENCY_FORMAT.format(balance + pendingAmount)));
        ...
        events.addEventBinding(CustomUIEventBindingType.Activating, "#QuickAll", EventData.of("Action", "QuickMax"), false);
    ...
    public void handleDataEvent(
        ...
        switch (action) {
            ...
            case "QuickMax":
                pendingAmount = 10000; // TODO: Get from player inventory
                sendUpdate();
                break;
            ...
            case "AmountChanged":
                ...
                if (inputAmount != null && !inputAmount.isEmpty()) {
                    try {
                        pendingAmount = Double.parseDouble(inputAmount.replace(",", "").replace("$", ""));
                    } catch (NumberFormatException e) {
                        pendingAmount = 0;
                    }
                } else {
            ...
            case "Confirm":
                if (pendingAmount > 0) {
                    boolean success = bankManager.deposit(uuidComponent.getUuid(), pendingAmount);
                    if (success) {
                        player.sendMessage(Message.raw("§a✓ Successfully deposited $" + CURRENCY_FORMAT.format(pendingAmount)));
                        ATMMenuPage menu = new ATMMenuPage(playerRef, bankManager);
                        player.getPageManager().openCustomPage(ref, store, menu);
                    } else {
                        player.sendMessage(Message.raw("§c✗ Deposit failed. Please try again."));
                        sendUpdate();
                    }
                } else {
