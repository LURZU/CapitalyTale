...
public class DepositCommand extends AbstractPlayerCommand {
    ...
    public DepositCommand(BankManager bankManager) {
        super("lurzudeposit", "Deposit money into your bank account (Admin/Test)");
        this.bankManager = bankManager;
        
        this.amountArg = this.withRequiredArg("lurzuamount", "Amount to deposit", ArgTypes.DOUBLE);
    }
    ...
    protected void execute(
        ...
        bankManager.deposit(uuidComponent.getUuid(), playerName, amount);
        double newBalance = bankManager.getBalance(uuidComponent.getUuid());
        
        player.sendMessage(Message.raw(
            "[CapitalyTale] Deposited " + String.format("%.2f", amount) + " coins"
        ));
        player.sendMessage(Message.raw(
